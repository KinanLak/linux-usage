...
function _extractPlanLabel(creds: any): string | null {
  const oauth = creds.claudeAiOauth || creds.claude_ai_oauth || {};
  const tier =
    creds.rateLimitTier || creds.rate_limit_tier || oauth.rateLimitTier || oauth.rate_limit_tier;
...
function _formatPlanLabel(value: string): string {
  const parts = value
    ...
    .filter((p: string) => p)
    .slice();
  if (parts.length && parts[0].toLowerCase() === "default") parts.shift();
