...
function readJsonFile(path: string): any {
    ...
    return JSON.parse(bytesToString(contents));
}
...
function writeJsonFile(path: string, data: any): void {
    ...
    file.replace_contents(
        stringToBytes(JSON.stringify(data, null, 2)),
        null,
        false,
...
function parseJwtClaims(token: string): any {
    ...
    const payloadSource = parts[1];
    if (!payloadSource) return null;
    let payload = payloadSource.replace(/-/g, "+").replace(/_/g, "/");
    while (payload.length % 4) payload += "=";
    try {
        const decoded = GLib.base64_decode(payload);
        return JSON.parse(bytesToString(decoded));
    } catch {
        return null;
    }
}
...
function runCommandWithInput(args: string[], input: string): { success: boolean; stdout: string; stderr: string } {
    try {
...
function percentPairFromUsedLimit(used: number | null, limit: number | null): [number | null, number | null] {
...
function percentPairFromRemainingLimit(remaining: number | null, limit: number | null): [number | null, number | null] {
...
function statusSnapshot(metadata: any, status: string, errorMessage?: string | null, remediation?: string | null): any {
