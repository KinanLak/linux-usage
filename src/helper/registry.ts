...
function _providerFetchers(): Record<string, (metadata: any) => any> {
  const { ClaudeProvider } = imports.providers.claude;
  const { CopilotProvider } = imports.providers.copilot;
  const { CodexProvider } = imports.providers.codex;
  return {
...
function _formatBackoffDuration(blockedUntil: string): string {
...
function _snapshotFromBackoff(metadata: any, cached: any | null, backoff: any): any {
  ...
  const message = `Provider temporarily backed off for ${duration} before retry after ${reason}`;
...
function _fetchProviderSnapshot(
  metadata: any,
  fetcher: (meta: any) => any,
  cached: any | null,
): any {
  ...
  const providers = catalog.map((metadata: any) => {
    ...
    if (!fetcher) {
      return Utils.statusSnapshot(
        metadata,
        "error",
        `No implementation for provider '${metadata.id}'`,
      );
    }
    return _fetchProviderSnapshot(metadata, fetcher, cached);
  });

  const hasError = providers.some(
    (p: any) => p.status === "error" || p.status === "auth_required",
  );
  const snapshot = {
...
function fetchOne(extensionDir: string, providerId: string): any | null {
  ...
  if (!fetcher) {
    return Utils.statusSnapshot(
      metadata,
      "error",
      `No implementation for provider '${providerId}'`,
    );
  }
...
var Registry = { fetchAll, fetchCachedOrLive, fetchOne };
