...
function _settings() {
    return ExtensionUtils.getSettings('org.kinanl.linux-usage');
}

function _createProxy() {
    return new Promise((resolve, reject) => {
        try {
            new HelperProxy(Gio.DBus.session, BUS_NAME, OBJECT_PATH, (instance, error) => {
                if (error) {
                    reject(error);
                    return;
                }
...
function _runCli(args) {
    return new Promise((resolve, reject) => {
        const helperPath = _settings().get_string('helper-path') || 'linux-usage-helper';
        const subprocess = Gio.Subprocess.new(
            [helperPath].concat(args),
...
async function _runMethod(methodName) {
    try {
        const proxy = await _createProxy();
        const result = await new Promise((resolve, reject) => {
            proxy[`${methodName}Remote`]((value, error) => {
                if (error) {
                    reject(error);
                    return;
                }
